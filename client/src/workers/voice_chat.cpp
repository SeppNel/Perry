...
static void underflow_callback(struct SoundIoOutStream *outstream) {
    static int count = 0;
    std::cerr << "VC | Underflow " << ++count << " (network latency too high)" << std::endl;
}
...
void network_send_thread() {
    ...
    std::cout << "VC | Disconnected from server\n";
...
void run() {
    ...
    // Destroy audio streams in correct order
